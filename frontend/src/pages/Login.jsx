...
export default function Login() {
  ...
  const handlePassageiroLogin = async (e) => {
    e.preventDefault();
        ...
        <div className="relative z-10 space-y-6">
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div
                key={s.l}
                className="bg-white/10 border border-white/10 rounded-2xl p-3 text-center"
              >
      ...
      <div className="flex-1 flex flex-col justify-center items-center p-8 bg-slate-50">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-slate-800">Acessar Portal</h2>
          ...
          <div className="flex bg-white border border-slate-200 rounded-2xl p-1 mb-6 shadow-sm">
            {[
              { key: 'passageiro', icon: '🧳', label: 'Passageiro' },
              { key: 'admin', icon: '👨‍✈️', label: 'Funcionário' },
            ].map((t) => (
              <button
                key={t.key}
                onClick={() => { setTab(t.key); setError(''); }}
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                  tab === t.key ? 'text-white shadow-md' : 'text-slate-500 hover:text-slate-700'
                }`}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
            {tab === 'passageiro' ? (
              <form onSubmit={handlePassageiroLogin} className="space-y-4">
                <div>
                ...
                >
                  {loading ? 'Autenticando...' : 'Entrar como Passageiro →'}
                </button>
              ...
              <form onSubmit={handleAdminLogin} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full disabled:bg-slate-400 disabled:cursor-not-allowed text-white py-3.5 rounded-xl font-semibold transition-opacity hover:opacity-90"
                  style={loading ? {} : { backgroundColor: '#0F2044' }}
                >
                  {loading ? 'Autenticando...' : 'Entrar como Funcionário →'}
                </button>
              </form>
            )}
          </div>
